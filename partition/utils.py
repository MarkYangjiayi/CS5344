...
from partition.logs import log_write_com_result, log_write_graph_info, print_difference
...
def get_communities(G, typology, k = 0, seed = 0):
    raw_partition = list()
    list_com = list()
    set_com = set()
    info = list()
...
def label_node_communities(compactGraph, communities, type_com, path, name):
    directGraph = nx.read_gml(f'{path}/Final_DiGraph_{name}.gml')

    for member in communities:
        compactGraph.nodes[member][f'{type_com}Comm'] = communities[member]
        directGraph.nodes[member][f'{type_com}Comm'] = communities[member]
        
    nx.write_gml(directGraph, f'{path}/Final_DiGraph_sentiment_{name}.gml')
    nx.write_gml(compactGraph, f'{path}/Final_Graph_sentiment_{name}.gml')
...
def community_detection(name, path, typology):
    graph = nx.read_gml(f'{path}/Final_Graph_{name}.gml')
    multi = nx.read_gml(f'{path}/Final_MultiGraph_{name}.gml')
    if typology == 'weight':
        log_write_graph_info(name, nx.info(graph), nx.info(multi))
    #### METIS
    list_com_metis, set_com_metis, info, exe_time = get_communities(graph, typology)
    mod_m = modularity(list_com_metis, graph, weight='weight')
    cov_m = coverage(multi, set_com_metis)
    log_write_com_result('Metis', info, mod_m, cov_m, exe_time, typology, name)

    label_node_communities(graph, list_com_metis, typology, path, name)
